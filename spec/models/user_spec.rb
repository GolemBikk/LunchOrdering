...
RSpec.describe User, type: :model do
  ...
  describe 'when save record' do
    before do
      FactoryGirl.create(:user, email: 'first@mail.com')
      FactoryGirl.create(:user, email: 'second@mail.com')
    end

    let(:first_user) { User.find_by_email('first@mail.com') }
    let(:not_first_user) { User.find_by_email('second@mail.com') }

    it { expect(first_user.is_admin).to be true }
    it { expect(not_first_user.is_admin).to be false }
  end
